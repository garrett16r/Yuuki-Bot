...
let addNewUser = (msg, con) => {
    return new Promise((resolve) => {
        let sql = `INSERT INTO users (id, experience) VALUES (?, ?)`;
        let values = [msg.author.id, 1]

        con.query(sql, values);
        console.log(`Added user ${msg.author.tag} to database!`);
        resolve();
    });
}
