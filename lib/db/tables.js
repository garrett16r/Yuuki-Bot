...
let checkData = () => {
    return new Promise((resolve, reject) => {
        db.getConnection((err, con) => {
            ...
            sql = 'SELECT EXISTS(SELECT * FROM pokemon) as dataExists'

            con.query(sql, (err, rows) => {
                if (err) reject(err);

                con.release();
                resolve(rows[0].dataExists);
            });
        });
    });
}
...
let createTables = async () => {
    return new Promise((resolve, reject) => {
        db.getConnection( async (err, con) => {
            ...
            // Check if data exists, then import pokemon data from .csv file, then set sprites
