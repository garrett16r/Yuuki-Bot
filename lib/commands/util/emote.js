...
module.exports = class TakeEmoteCommand extends Command {
    constructor (client) {
      ...
      async run(msg, { emote, name }) {
        const channel = this.client.channels.cache.get(COMMANDS_ID);
        var fileExt = "";
        ...
        msg.guild.emojis.create(emoteURL, name)
          .catch(err => {
            return msg.reply("could not that add emote to the server!");
          });

        channel.send(`Emote ${name} added to the server!`);
      }
}